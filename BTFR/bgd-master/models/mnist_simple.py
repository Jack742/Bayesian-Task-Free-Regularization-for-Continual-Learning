...
class MNISTCOMPARE(nn.Module):
    def __init__(self, **kwargs):
        super(MNISTCOMPARE, self).__init__()
        layer1_width = kwargs.get("layer1_width", 512)
        ...
        self.layer1 = nn.Sequential(nn.Linear(self.input_size, layer1_width))
        self.last_layer = nn.ModuleList([nn.Linear(layer1_width, self.num_of_classes)
                                         for _ in range(0, self.num_of_datasets)])
...
def mnist_compare(**kwargs):
    return MNISTCOMPARE(input_size=784, layer1_width=512, num_of_datasets=1,
